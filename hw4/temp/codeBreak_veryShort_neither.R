...
setwd("/home/cocodong/class/hw4/veryShort")
...
codeBreak <- function(fileHeader, decodedFileHeader, selector, iteration, randomID){
    fileOut = paste(fileHeader,"_", selector, "_", iteration, "_", randomID, ".txt", sep="")
...
fileHeaders = "CodedMessage_veryshort.txt"
decodedFileHeaders = "DecodedMessage_veryshort.txt"
selectors = c("row", "column", "all", "neither")
iterations = c(100, 500, 1000, 5000, 10000)
myrepeatTimes = 1
randomId = sample(1:100000, 1)

codeBreak(fileHeaders, decodedFileHeaders, selectors[4], iterations[4], randomId)
